...
export default function GoalsModal() {
  ...
  const handleSubmit = async () => {
    ...
    // const res = await fetch("/api/goals", { method: "POST", body: JSON.stringify(payload) });
    // const data = await res.json();

    const existing = JSON.parse(localStorage.getItem("goals") || "[]");
    existing.push({ id: Date.now(), ...payload });
    console.log("Goal saved:", payload); // stockage temporaire
  ...
  return (
    <div className="flex font-sans">
        <button className="btn btn-outline btn-accent border-(--color-base-700) text-base-content bg-base-300 hover:border-accent ease-in duration-300" onClick={()=>(document.getElementById('my_modal_5') as HTMLDialogElement)?.showModal()}><Plus /> save goal</button>
        <dialog id="my_modal_5" className="modal modal-bottom sm:modal-middle">
          <div className="modal-box flex flex-col gap-6">
            <h3 className="font-bold text-lg">You can set up a goal !</h3>
            <p className="py-4">Set up the deadline with the price of your goal so the app will help you to manage your budget !</p>
            ...
            <input
              ...
              className={`input input-accent w-60 text-center ${sumbit && goalName.trim() === "" ? "input-error" : ""}`}
              value={goalName}
            ...
            <input
              ...
              required
              placeholder="Price of your goal ? 10€"
              min="0"
              max="10000000"
